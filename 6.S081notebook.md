...
read也可以用来读文件,到文件结尾, 那就返回0 . 文件描述符不存在, 返回-1.unix约定 程序成功退出返回0 
...
wait system call是啥?是让父进程等子进程结束. 下面这个代码跨了一个figure导致我看半天看不出来

```c
int pid = fork();
if(pid > 0){
	printf("parent: child=%d\n", pid);//pid=19的意思是 xv6系统启动以来的19个进程.
...
父进程fork 会返回子进程的pid, 子进程fork会返回0 , 所以尽管子进程复制了data,还是可以分辨他们.fd表也会复制, 所以父进程打开 的话,子进程会看到相同的一组文件描述符.

他会让父进程和子进程 虚拟内存映射一样.

他会复制哪些呢?  fork()之前的代码会复制吗?  我觉得应该不会, 不然不是无限复制?O 不会无限复制, 如果是子进程那就pid == 0 了; 那应该就是复制所有代码?

如果调用fork多次,需要等待所有子进程, 所以必须调用wait  多次,每次调用wait都会在一个子进程退出时返回.wait会返回子进程的pid. 以此来区分.
...
argv[1] = "hello";
argv[2] = 0;
exec("/bin/echo", argv);// 操作系统从/bin/echo 中加载指令, 替代当前进程的内存, 然后开始执行那些指令.  相当于从终端输入echo hello然后回车 
printf("exec error\n");//exec一般返回, 因为替代当前进程的内存,除非内核态执行时发生错误返回-1.
```

系统调用exec 加载了/bin/echo , 所以这整个程序相当于echo程序.
...
vim打开sh.c, 输入143 再回车就可以进入143行. 如下
...
```c
   if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      // Chdir must be called by the parent, not the child.
       ...
       //78行
   void    runcmd{ //shell中包装了exec
           switch(cmd->type){
           case EXEC:
           exec(ecmd->argv[0],ecmd->argv);//把第一个参数加载命令然后执行, 
       }
```
...
答: 因为我们不想让echo替代shell, 不然echo结束就退出shell进程了, 所以我们fork一个进程参考下面的例子forkexec.c
...
main(){
    ...
    pid = fork();
    if(pid == 0){  //是子进程
        char *argv[] = { "echo","This","echo",0};
        exec("echo",argv);//echo命令替换他自己, 做完echo然后退出.
        printf("exec failed!\n");//如果exec系统调用出错,就会运行到这里,比如没有echo命令
        exit(1);//操作系统会把这个1传递给下面wait的父进程.
    }
    else{
        printf("parent waiting !!\n");
        wait(&status);// 这个方法可以知道调用进程的退出状态, shell进程等待子进程结束,然后再输出$符号继续让用户输入
...
这里大部分复制的内存都被丢弃并被exec替换, 之后的lab会有一些优化 ,比如 copy on write fork, 他使用了虚拟内存的技巧 lazy copy, 如果 fork后面跟着exec  , 或子进程不使用复制的内存, 则不用复制. 
...
pid = fork();
if(pid == 0 ){
    close(1);//子进程改变fd ,这是unix一种常用的做法, 我们不想改变shell的输出, 只是重定向子进程的output 对echo的输出重定向, 释放文件描述符1
    //int close(int fd) Release open file fd.
 //int open(char *file, int flags) Open a file; flags indicate read/write; returns an fd (file descriptor). 
    open("output.txt", 0_WRONLY|0_CREATE );//一定是刚刚被释放的1号fd ,0号fd依然连接到控制台.
     char *argv[] = { "echo","This","redicted",0};
     exec("echo",argv);//echo依旧输出到fd1
     printf("exec failed!\n");
     exit(1);//操作系统会把这个1传递给下面wait的父进程.
}else{
 wait((int *) 0);
